...
def menu():
    user_id = input("Ingrese su ID de usuario para iniciar sesión: ")
    if not user_id.isdigit():
        print("ID de usuario inválido. Debe ser un número entero. Intente nuevamente.")
        return
    else:
        esAdmin = validacionAdmin(int(user_id))
        if not esAdmin:
            ...
            while True:
                ...
                if opcion == "1":
                    ...
                    try:
                        ...
                        prepararHabitacion(reserva_id, user_id)
                    except ValueError:
